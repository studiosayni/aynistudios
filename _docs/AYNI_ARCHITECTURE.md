...
**2026-07-27 — image optimizer, payload, real-device pass.** `/_next/image` had 404'd since launch; the cause was our own config, not the platform (see IMAGE HANDLING). Library thumbnails now come from our origin, 1280KB → 228KB on a phone. Hero stills load just-in-time (629KB → 247KB at 3.0s) and the partner wall is 3-up on phones. A cold homepage is 2.46MB / 27 requests, **55% of it the hero word-cloud video** — the remaining lever, sized in `_docs/tasks.md`. All confirmed on real mobile and desktop hardware.
...
⚠️ **Only the first hero still is allowed to load on the critical path.** `loadedThrough` gates which of the four gets a `src`, advanced off the rotation tick (index mirrored into a ref — advancing both from an effect body trips `react-hooks/set-state-in-effect`), plus one 3.5s timer for the second still, which has no tick preceding it. That gives 4s of lead on a 1.6s crossfade. Rendering all four unconditionally costs 629KB before the second is needed at 7.5s; the gate makes it 247KB at 3.0s. Only still 0 carries `fetchPriority="high"` — the rest are `"low"` so they never contend with it. Measured 2026-07-27; the hero video is separately 1358KB, so the homepage is video-dominated and this is the second lever, not the first.
...
- **`scripts/build-brand-assets.py` pre-builds WebP for everything in `public/brand/`.** Run it and commit the output after adding or replacing any brand asset. Originals are kept deliberately — the hero JPEGs are the `<img src>` fallback, and the 1080px logo PNG is still what the schema.org `logo` field points crawlers at (they want a large, universally supported file, not a 160px WebP). Full-bleed backgrounds are driven by the `SRCSET_SOURCES` list of `(folder, glob)` pairs — add a row to give a new backdrop the same treatment.
  ...
  - *Partner logos*: single WebP each, max 480px, q82 (line art and type need more than photos). The set went 426KB → ~87KB. The 480px ceiling still covers the largest chip (186px at `md`) past 2x. Note the wall is **3 columns below `sm`, 4 at `sm`, 6 at `md`** with `h-16`/`h-20`: at two columns the twelve chips stacked six rows deep, ~560px of the brightest surface on the page, since each carries its own white plate.
