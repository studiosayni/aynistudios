...
**Share cards with a photo (2026-09-18):** Noah: link previews showed a dark text card, "nothing significant". `app/share/[slug]/route.tsx` now draws a 1200×630 still behind the title (`public/brand/og/<slug>.jpg`, home/work/services/studio/contact/los-angeles/guides plus one per service; anything else falls back to the home boat) with two scrims and the amber rule, set in Barlow (Barlow-Regular/Bold.ttf, OFL, in `app/share/fonts/`; Satori ships no font). Satori gotchas met: `inset` is not supported (use top/left/width/height), and webp is not, hence the JPEG crops. The route is `force-static` + `dynamicParams=false`, so the file reads happen at build, never on App Hosting at runtime. `seo.ts` appends `?v=2` to the og:image URL to bust LinkedIn/WhatsApp/Slack/iMessage caches; bump it on the next card redesign. Case-study and film cards still use the home still; per-project stills would need clean local frames (their images are YouTube thumbnails).
