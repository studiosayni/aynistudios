...
- `InquiryCTA` gained an optional `image` prop that splits the band: amber copy left, full-height still right with a caption. The homepage passes the Seafood Souq fishing boat (`seafood-souq-boat-1920.webp`); every other page keeps the plain amber band.
